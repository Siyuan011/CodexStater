...
function filter(items,state,monitorId){return items.filter(t=>(!state.account||(t.account||"__unknown__")===state.account)&&(!state.model||t.model===state.model)&&(!state.effort||effort(t)===state.effort)&&(!state.excludeMonitor||t.id!==monitorId)&&(!state.excludeInternal||!t.internal)&&(!state.task||t.id===state.task));}
